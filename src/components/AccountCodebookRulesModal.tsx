...
// codebook rules" da Account Codebooks (aberto a partir do RecreateCodebookModal
// quando o usuário confirma "Generate codebook rules").
//
// Decisão de arquitetura: por escolha do PM, este é uma CÓPIA STANDALONE do
// ValidatorCodesModal da aba Validator — deliberadamente NÃO compartilha
// componente nem stylesheet com ele (ver accountCodebookRulesModal.css). A UI e
// as interações iniciais são idênticas ao Validator (fonte da verdade de layout,
// espaçamento, campos, ações e padrões de interação), mas as duas experiências
// devem evoluir de forma independente. Ao portar, além do CSS re-escopado:
//   - virou um modal com isOpen/onClose, ESC e clique-fora para fechar (convenção
//     dos modais da Account Codebooks), sobre um overlay `fixed` no app inteiro,
//     em vez do overlay `absolute` dentro da aba do protótipo;
//   - os IDs globais foram renomeados de #aiCoder-dialog-prompt-multiline-* para
//     #account-codebook-rules-* para não colidirem com o Validator.
//
// Recurso SÓ de protótipo/validação — não faz parte do produto real. Estático:
// os botões e campos são apenas visuais, sem fluxo conectado.
// -----------------------------------------------------------------------------
...
interface AccountCodebookRulesModalProps {
     ...
     *  true, o pai nomeia o registro como "{origem} – AI trained" e marca
     *  trained; caso contrário mantém a convenção "{origem} - AI". */
    onCreate: (options?: { trained?: boolean }) => void;
}

// Sufixo anexado à regra de um code refinado pelo Quality Check. Serve tanto de
// texto visível quanto de marcador para não reanexar em re-runs.
const REFINE_MARKER = 'Refined after quality check.';
...
function AccountCodebookRulesModal({ isOpen, onClose, onCreate }: AccountCodebookRulesModalProps) {
    ...
    const applyQualityCheck = (codesToRefine: string[]) => {
        ...
        if (codeSet.size > 0) {
            ...
            const nextRows = rows.map((row) => {
                ...
                return { ...row, rule: `${row.rule} ${REFINE_MARKER}` };
            });
            dispatch({ type: 'commit', rows: nextRows });
    ...
    // Fecha com ESC (convenção dos modais da Account Codebooks). Enquanto o
    // painel de QC está aberto, o ESC é tratado por ele (fecha o painel, não o
    // modal inteiro).
    useEffect(() => {
        if (!isOpen) return;
        const onKey = (e: KeyboardEvent) => {
            if (e.key === 'Escape' && qcPhase === 'idle') onClose();
        };
        document.addEventListener('keydown', onKey);
        return () => document.removeEventListener('keydown', onKey);
    }, [isOpen, onClose, qcPhase]);
    ...
    return (
        <>
        <div
            className="account-codebook-rules-overlay"
            role="dialog"
            aria-modal="true"
            aria-label="Generate codebook rules"
            onMouseDown={onClose}
        >
                ...
                <div className="modal-content">
                    ...
                    <div className="modal-header dialogHeader">
                        ...
                        <div className="aiCoder-dialog-prompt-multiline-codebook-tools">
                            <input
                                id="account-codebook-rules-search"
                                type="search"
                                placeholder="Search..."
                                autoComplete="off"
                                spellCheck={false}
                            ...
                            <div className="aiCoder-dialog-prompt-multiline-show-rules-group">
                                <input
                                    ...
                                    type="checkbox"
                                    id="account-codebook-rules-show-rules"
                                    checked={showRules}
                                    onChange={(e) => setShowRules(e.target.checked)}
                                />
                                <label id="account-codebook-rules-show-rules-label" htmlFor="account-codebook-rules-show-rules">
                                    View code rules
                                </label>
                            </div>
                        </div>
                    ...
                    <div className="modal-body">
                        ...
                        <div className="form-group">
                            ...
                            <div
                                id="account-codebook-rules-html"
                            ...
                            >
                                {rows.map((row, index) =>
                                    row.kind === 'net' ? (
                                        ...
                                        <div
                                            key={index}
                                            className={`codebook-code${selectedIndices.has(index) ? ' selected' : ''}${draggingIndex === index ? ' dragging' : ''}${mergeTargetIndex === index ? ' merge-target' : ''}${refinedLabels.has(row.label) ? ' refined' : ''}`}
                                            contentEditable={false}
                                            onClick={(e) => {
                                                e.stopPropagation();
                                        ...
                                        >
                                            ...
                                            {refinedLabels.has(row.label) && (
                                                <span
                                                    className="codebook-refined-badge"
                                                    contentEditable={false}
                                                    title="Rule refined by quality check"
                                                >
                                                    Refined
                                                </span>
                                            )}
...
export default AccountCodebookRulesModal;
